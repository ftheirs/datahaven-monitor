...
const HEAVY_CONFIG = {
	...
	deleteAbsentPollTotalMs: 26 * 60_000, // 26 min
} as const;
		...
		try {
			...
			for (const fk of fileKeys) {
				...
				if (prev !== "ready" && status === "ready") {
					console.log(`[monitor-heavy] ${label} file became ready: ${fk}`);
				}
...
async function waitUntilFilesAbsentViaTree(
	...
	const stillPresent = fileKeys.filter((fk) =>
		lastPresent.get(fk.toLowerCase()),
	);
	console.log(
	...
	try {
		...
		async function issueStorageRequestSeq(
			...
			for (let i = 0; i < files.length; i++) {
				...
				console.log(
					`[monitor-heavy] [SR ${idx}/${files.length}] tx sent: ${tx}`,
				);
			...
			await withConcurrency(
				...
				async (item) => {
					...
					await waitForOnChainData(
						() =>
							userApi.query.fileSystem.storageRequests(
								f.fileKeyHex as `0x${string}`,
							),
						(sr) => !(sr as any).isNone,
						{
							retries: Math.ceil(
								HEAVY_CONFIG.storageRequestVisibleTimeoutMs / 5_000,
							),
							delayMs: 5_000,
						},
					);
				...
				while (true) {
					...
					await withConcurrency(
						...
						async (fk) => {
							...
							last.set(
								fk.toLowerCase(),
								(sr as any).isNone ? "Cleared" : "Present",
							);
						},
					);
			...
			for (
				...
				for (let i = 0; i < remaining.length; i++) {
					...
					try {
						...
						const fi = await mspClient.files.getFileInfo(bucketId, fk);

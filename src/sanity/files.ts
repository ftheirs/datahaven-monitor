...
export async function runFileUploadCheck(
  ...
  const normalizedBucketId = bucketId.startsWith("0x") ? bucketId : `0x${bucketId}`;
...
export async function runIssueStorageRequest(
  ...
  replicas: number,
): Promise<{ fileKey: `0x${string}`; fingerprint: `0x${string}`; fileBlob: Blob }> {
  ...
  // Derive fileKey using FileManager + substrate types.
  const owner = registry.createType("AccountId20", viem.account.address) as AccountId20;
  const bucketIdH256 = registry.createType("H256", bucketIdHex) as H256;
  const fileKeyH256 = await fileManager.computeFileKey(owner, bucketIdH256, fileLocation);
  const fileKey = normalizeHex(fileKeyH256.toString());
...
export function generateFileLocation(filename: string): string {
  const base =
    FILE_LOCATION_BASES[
    Math.floor(Math.random() * FILE_LOCATION_BASES.length)
    ];
  if (!base) {
    return `/${filename}`;
  }
